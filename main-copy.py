...
def draw_blocks(blocks):
	for r in range(ROWS):
		for c in range(COLUMNS):
			if blocks[r][c].water_level<=FULL_WATER_LEVEL/100:
				blocks[r][c].state=EMPTY_STATE
				# blocks[r][c].water_level=0
...
def apply_rule1(block, block_below):
	if block_below.state==EMPTY_STATE:
		block_below.state = WATER_STATE
		block_below.water_level += block.water_level
		...
		if (empty_space_below>=block.water_level):
			...
			block_below.water_level += empty_space_below
			block.water_level -= empty_space_below

def apply_rule2(block_left, block, block_right):
	...
	block_left.water_level += block_left_water_flow
	block.water_level += block_water_flow
	block_right.water_level += block_right_water_flow
